...
class Simulator(object):
    ...
    def run(self, tolerance=0.02, n_test=0):
        ...
        if status:
            ...
            if status['violation'] == 0: # Legal
                if status['action'] == None:
                    ...
                    self.screen.blit(self.font.render("fAgent drove {}. (rewarded {:.2f})".format(status['action'], status['reward']), True, self.colors['dgreen'], self.bg_color), (350, 40))
            else: # Illegal
                if status['action'] == None:
