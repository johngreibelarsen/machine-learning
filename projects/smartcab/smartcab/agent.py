...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        if (self.Q.has_key(state)):
            ...
            maxQ = max(actionsAndQsForState.values())    
        else:
    ...
    def createQ(self, state):
        ...
        if (self.learning == True and not self.Q.has_key(state)):
            newActionsAndQs = dict()
            for i in self.valid_actions:
                newActionsAndQs[i] = 0.0
            self.Q[state] = newActionsAndQs
            
        return


    def action_for_highestQ(self, state):
        """ Chooses an action with the highest Q-value for the current state. In case multiple 
            actions exists for same max Q then a randomly choosen action is returned from the max list. """
    ...
    def learn(self, state, action, reward):
        ...
        if (self.learning == True and self.Q.has_key(state)):
            actionsAndQsForState = self.Q.get(state)
            oldQ = actionsAndQsForState[action]
            """a discount factor of 0 has been assumed make the agent "myopic" 
                (or short-sighted) by only considering current rewards """
            newQ = oldQ + self.alpha*(reward - oldQ) 
            actionsAndQsForState[action] = newQ
